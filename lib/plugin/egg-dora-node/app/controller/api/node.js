...
let NodeController = {
    ...
    checkNodeFormData(ctx, fields) {
        ...
        if (!validatorUtil.isRegularCharacter(fields.title)) {
            errMsg = ctx.__("validate_error_field", [ctx.__("label_node_title")]);
        }
        if (!validator.isLength(fields.title, 2, 50)) {
            errMsg = ctx.__("validate_rangelength", [ctx.__("label_node_title"), 2, 50]);
        }
        // if (fields.stitle && !validator.isLength(fields.stitle, 2, 50)) {
        //     errMsg = ctx.__("validate_rangelength", [ctx.__("label_node_stitle"), 2, 50]);
        // }
        if (!fields.categories) {
            errMsg = ctx.__("validate_userNode_category");
        }
        // if (!fields.tags) {
        //     errMsg = ctx.__("validate_selectNull", [ctx.__("label_node_tags")]);
        // }

        if (!validator.isLength(fields.discription, 5, 300)) {
            errMsg = ctx.__("validate_rangelength", [ctx.__("label_node_dis"), 5, 300]);
        }

        if (fields.content && !validator.isLength(fields.content, 5, 100000)) {
            errMsg = ctx.__("validate_rangelength", [ctx.__("label_node_content"), 5, 100000]);
        }
    ...
    renderNodeList(ctx, userId = "", nodeList = []) {
        ...
        return new Promise(async (resolve, reject) => {
            try {
                ...
                for (let nodeItem of newNodeList) {
                    nodeItem.id = nodeItem._id;
                    nodeItem.hasPraised = false;
                    nodeItem.hasComment = false;
                    nodeItem.hasFavorite = false;
                    nodeItem.hasDespise = false;
                    nodeItem.uAuthor && (nodeItem.uAuthor.had_followed = false);

                    if (!_.isEmpty(userInfo)) {
                        // 本人是否已点赞
                        if (userInfo.praiseNodes && userInfo.praiseNodes.indexOf(nodeItem._id) >= 0) {
                            nodeItem.hasPraised = true;
                        }
                        // 本人是否已收藏
                        if (userInfo.favorites && userInfo.favorites.indexOf(nodeItem._id) >= 0) {
                            nodeItem.hasFavorite = true;
                        }
                        // 本人是否已踩
                        if (userInfo.despises && userInfo.despises.indexOf(nodeItem._id) >= 0) {
                            nodeItem.hasDespise = true;
                        }
                        // 本人是否已留言
                        // let nodeMessage = await ctx.service.message.item(ctx, {
                        //     query: {
                        //         nodeId: nodeItem._id,
                        //         author: userInfo._id
                        //     }
                        // })
                        // if (!_.isEmpty(nodeMessage)) {
                        //     nodeItem.hasComment = true;
                        // }
                        // 本人是否已关注作者
                        // if (userInfo.watchers.length > 0 && nodeItem.uAuthor && userInfo.watchers.indexOf(nodeItem.uAuthor._id) >= 0) {
                        //     nodeItem.uAuthor.had_followed = true;
                        // }
                    }
                    ...
                    nodeItem.commentNum = commentNum;

                    // 点赞总数
                    let likeNum = await ctx.service.user.count({
                        praiseNodes: nodeItem._id
                    })
                    nodeItem.likeNum = likeNum;

                    // 收藏总数
                    let favoriteNum = await ctx.service.user.count({
                        favorites: nodeItem._id
                    })
                    nodeItem.favoriteNum = favoriteNum;

                    // 踩帖总数
                    let despiseNum = await ctx.service.user.count({
                        despises: nodeItem._id
                    });
                    nodeItem.despiseNum = despiseNum;

                    // if (nodeItem.simpleComments) {
                    //     nodeItem.simpleComments = JSON.parse(nodeItem.simpleComments);
                    // }

                    // 处理用户敏感信息
                    nodeItem.uAuthor && siteFunc.clearUserSensitiveInformation(nodeItem.uAuthor);
                }
    ...
    /**
     * @api {get} /api/node/getList 查询帖子列表
     * @apiDescription 根据参数获取对应的帖子列表,默认按时间查询，可作为发现栏目列表
     * @apiName /node/getList
     * @apiGroup Node
     ...
     *    ]
     *}
     * @apiSampleRequest http://localhost:8080/api/node/getList
     * @apiVersion 1.0.0
     */

    /**
     * @api {get} /api/node/getUserNodes 查询指定用户的节点列表
     * @apiDescription 查询指定用户的节点列表，带分页
     * @apiName /node/getUserNodes
     * @apiGroup Node
     * @apiParam {string} current 当前页码
     * @apiParam {string} pageSize 每页记录数
     ...
     *  ]
     *}
     * @apiSampleRequest http://localhost:8080/api/node/getUserNodes
     * @apiVersion 1.0.0
     */
    async list(ctx, app) {
        ...
        try {
            ...
            let nodeList = await ctx.service.node.find(payload, {
                ...
                files: getNodeListFields(filesType)
            });

            nodeList.docs = await this.renderNodeList(ctx, userInfo._id, nodeList.docs);
    ...
    async getTopIndexNodes(ctx, app) {
        ...
        try {
            ...
            if (!_.isEmpty(userInfo) && !_.isEmpty(userInfo.watchTags) && userInfo.watchTags.length > 0) {
                ...
                let recNodesNum = await ctx.service.node.count(tagQuery);
                recNodes = await ctx.service.node.find(payload, {
                    query: tagQuery,
                    files: getNodeListFields(),
                    sort: sortObj
                })

                if (recNodesNum > current * pageSize) {
                    recNodes.docs = await this.renderNodeList(ctx, userInfo._id, recNodes.docs);
                    ctx.helper.renderSuccess(ctx, {
                        data: recNodes
                    });
                } else {

                    let leftNormalSize = current * pageSize - recNodesNum;
                    if (leftNormalSize <= pageSize) {

                        if (leftNormalSize > 0) {
                            let leftNodes = await ctx.service.node.find({
                                current: 1,
                            ...
                            recNodes = _.concat(recNodes, leftNodes);
                        }
                    ...
                    recNodes.docs = await renderNodeList(ctx, userInfo._id, recNodes.docs);
                ...
                let nodes = await ctx.service.node.find(payload, {
                    query: queryObj,
                    files: getNodeListFields(),
                    sort: sortObj
                })
                nodes.docs = await this.renderNodeList(ctx, userInfo._id, nodes.docs);
    ...
    /**
     * @api {get} /api/node/getRadomNodes 获取随机节点列表
     * @apiDescription 获取随机节点列表 不分页
     * @apiName /node/getRadomNodes
     * @apiGroup Node
     * @apiSuccess {json} result
     * @apiSuccessExample {json} Success-Response:
     *{
     ...
     *    ]
     *}
     * @apiSampleRequest http://localhost:8080/api/node/getRadomNodes
     * @apiVersion 1.0.0
     */
    async getRadomNodes(ctx, app) {
        ...
        try {
            ...
            const totalNodes = await ctx.service.node.count(queryObj);

            randomArticles = await ctx.service.node.find(_.assign(payload, {
                skip: Math.floor(totalNodes * Math.random())
            }), {
                query: queryObj,
    ...
    /**
     * @api {get} /api/node/getNode 查询帖子详情
     * @apiDescription 查询帖子详情(只普通帖子或专题)
     * @apiName /api/node/getNode
     * @apiGroup Node
     ...
     *       {
     *        "type": "nodes",
     *        "node": "做一名研究人员可能会让你的人生非常充实并得到他人的认可。但我知道很多学生在做研究时受到前景的压力，一时陷入工程的舒适区。他们通常把这个阶段视为个人失败，觉得自己“不够优秀”。而根据我个人的经验，这从来就不是个人价值或者天赋的问题：在研究中成长需要某种不同的气质，这种气质往往与工程师成长的原因有些矛盾。以下是我见过的研究人员在职业生涯的某个阶段不得不面对的一些主要压力：\n  \n做研究要解决的是有多个答案（或没有答案）的不适定问题\n                               *        "
     *       },
     *       {
     *        "type": "video",
     *        "node": "http://creatorchain.oss-cn-hongkong.aliyuncs.com/upload/images/1071402816293179392.mp4"
     *       },
     *       {
     *        "type": "nodes",
     ...
     *       {
     *        "type": "image",
     *        "node": "http://creatorchain.oss-cn-hongkong.aliyuncs.com/upload/images/1075054319700676608.png"
     *       },
     *       {
     *        "type": "nodes",
     ...
     *   }
     * }
     * @apiSampleRequest http://localhost:8080/api/node/getNode
     * @apiVersion 1.0.0
     */
    async getOneNode(ctx, app) {
        ...
        try {
            ...
            await ctx.service.node.inc(ctx, targetId, {
                'clickNum': 1
            })
            // 如果是练习模式则将练习次数加1
            if (isPractise) {
            ...
            let renderNode = Array(targetNode);
            renderNode = await this.renderNodeList(ctx, userInfo._id, renderNode);
    ...
    /**
     * @api {get} /api/node/getNearbyNode 获取当前节点的上一篇和下一篇
     * @apiDescription 获取随机节点列表 不分页
     * @apiName /node/getNearbyNode
     * @apiParam {string} id 帖子Id
     * @apiGroup Node
     * @apiSuccess {json} result
     * @apiSuccessExample {json} Success-Response:
     ...
     *  "message": ""
     *}
     * @apiSampleRequest http://localhost:8080/api/node/getNearbyNode
     * @apiVersion 1.0.0
     */
    async getNearbyNode(ctx, app) {

        try {
            let nodeId = ctx.query.id;

            if (!nodeId || !shortid.isValid(nodeId)) {
                throw new Error(ctx.__('validate_error_params'));
            }
            ...
            let preNode = await ctx.service.node.find({
                isPaging: '0',
                pageSize: 1
            }, {
                query: {
                    ...
                    state: '2',
                    updateDate: {
                        "$lte": new Date(targetNode.updateDate)
                    }
                },
                files: 'title _id id data updateDate sImg discription'
            });

            let nextNode = await ctx.service.node.find({
                isPaging: '0',
                // pageSize: 1
            }, {
                query: {
                    ...
                    state: '2',
                    updateDate: {
                        "$gte": new Date(targetNode.updateDate)
                    }
                },
                sort: {
                    updateDate: 1
            ...
            ctx.helper.renderSuccess(ctx, {
                data: {
                    preNode: !_.isEmpty(preNode) ? preNode[0] : [],
                    nextNode: !_.isEmpty(nextNode) ? nextNode[0] : []
                }
            });
    ...
    /**
     * @api {get} /api/node/getMyFavoriteNodes 获取我收藏的帖子列表
     * @apiDescription 获取我收藏的帖子列表 带分页
     * @apiName /node/getMyFavoriteNodes
     * @apiGroup Node
     * @apiParam {string} current 当前页码
     * @apiParam {string} pageSize 每页记录数
     * @apiParam {string} token 登录时返回的参数鉴权
     ...
     *    ]
     *}
     * @apiSampleRequest http://localhost:8080/api/user/getMyFavoriteNodes
     * @apiVersion 1.0.0
     */
    async getMyFavoriteNodes(ctx, app) {
        ...
        try {
            ...
            let favoriteNodesData = await ctx.service.node.find(payload, {
                query: queryObj,
                searchKeys: ['name'],
                files: getNodeListFields()
            })

            favoriteNodesData.docs = await this.renderNodeList(ctx, userInfo._id, favoriteNodesData.docs);

            ctx.helper.renderSuccess(ctx, {
                data: favoriteNodesData
            });
     ...
     *    }
     *}
     * @apiSampleRequest http://localhost:8080/api/node/addOne
     * @apiVersion 1.0.0
     */
    async addNode(ctx, app) {
        ...
        try {
            ...
            this.checkNodeFormData(ctx, fields);
            ...
            // 设置显示模式
            // let checkInfo = siteFunc.checkNodeType(nodeFormObj.simpleComments);
            // nodeFormObj.appShowType = checkInfo.type;
            // nodeFormObj.imageArr = checkInfo.imgArr;
            // nodeFormObj.videoArr = checkInfo.videoArr;
            // if (checkInfo.type == '3') {
            //     nodeFormObj.videoImg = checkInfo.defaultUrl;
            // }

            // nodeFormObj.simpleComments = siteFunc.renderSimpleNode(nodeFormObj.simpleComments, checkInfo.imgArr, checkInfo.videoArr);

            // TODO 临时控制普通用户添加1天内不超过30篇
            // let rangeTime = getDateStr(-1);
            // let hadAddNodesNum = await ctx.service.node.count({
            ...
            // if (hadAddNodesNum > 30) {
            //     throw new Error(ctx.__("validate_forbid_more_req"));
            // }

            // nodeFormObj.comments = xss(fields.comments);
            // nodeFormObj.tags = Array(nodeFormObj.tags);
            // nodeFormObj.stitle = nodeFormObj.title;
            // nodeFormObj.uAuthor = ctx.session.user._id;
            // if (fields.draft == '1') {
            //     nodeFormObj.state = '0'
            // } else {
            //     nodeFormObj.state = '1'
            ...
            let newNode = await ctx.service.node.create(nodeFormObj);
    ...
    /**
     * @api {post} /api/node/updateOne 更新节点
     * @apiDescription 更新节点
     * @apiName /node/updateOne
     * @apiParam {string}  token 登录时返回的参数鉴权
     ...
     *    }
     *}
     * @apiSampleRequest http://localhost:8080/api/node/updateOne
     * @apiVersion 1.0.0
     */
    async updateNode(ctx, app) {
        ...
        try {
            ...
            // 设置显示模式
            // let checkInfo = siteFunc.checkNodeType(nodeObj.simpleComments);
            // nodeObj.appShowType = checkInfo.type;
            // nodeObj.imageArr = checkInfo.imgArr;
            // nodeObj.videoArr = checkInfo.videoArr;

            // nodeObj.simpleComments = siteFunc.renderSimpleNode(nodeObj.simpleComments, checkInfo.imgArr, checkInfo.videoArr);

            // if (checkInfo.type == '3') {
            //     nodeObj.videoImg = checkInfo.defaultUrl;
            // }

            // nodeObj.comments = xss(fields.comments);
            // nodeObj.stitle = nodeObj.title;
            // nodeObj.uAuthor = ctx.session.user._id;
            ...
            // }
            nodeObj.author = '';
            nodeObj.updateDate = new Date();

            await ctx.service.node.update(ctx, fields.id, nodeObj);
    ...
    // 随机获取图片
    async getRandomNodeImg(ctx, app) {
        ...
        try {
            ...
            // const totalNodes = await ctx.service.node.count(queryObj);
            // let randomArticles = await ctx.service.node.find(_.assign(payload, {
            //     isPaging: '0',
            //     pageSize,
            //     skip: Math.floor(totalNodes * Math.random())
            // }), {
