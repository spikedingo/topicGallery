<template>
  <div class="dr-toolbar">
    <el-col :xs="12" :md="6" class="option-button">
      <el-button size="small" type="primary" plain @click="addNode('node')" round>
        <svg-icon icon-class="icon_add" />
      </el-button>
      <el-button size="small" type="danger" plain round @click="branchDelete('node')">
        <svg-icon icon-class="icon_delete" />
      </el-button>
      <!-- <el-tooltip class="item" effect="dark" content="分配文章到用户" placement="top">
        <el-button size="small" type="warning" plain @click="directUser('node')" round>
...
<script>
import { deleteNode, regUserList } from "@/api/node";
export default {
  props: {
    device: String,
    pageInfo: Object,
  ...
  methods: {
    addNode() {
      ...
      this.$router.push(this.$root.adminBasePath + "/node/addNode");
    },
    directUser() {
      this.$store.dispatch("node/showDirectUserForm");
    },
    branchDelete(target) {
      let _this = this;
    ...
    changeUserOptions(value) {
      this.$store.dispatch("node/getNodeList", { userId: value });
    },
    changePostOptions(value) {
      if (value == "0") {
        this.$store.dispatch("node/getNodeList");
      } else if (value == "1") {
        this.$store.dispatch("node/getNodeList", { state: "1" });
      }
    }
    // TOPBARLEFTOPTION
  },
